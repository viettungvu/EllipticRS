...
{
    public class PhimRepository : IESRepository
    {
        ...
        public PhimRepository(string modify_index)
        {
        ...
        private static PhimRepository _instance;
        public static PhimRepository Instance
        {
            get
            {
                if (_instance == null)
                {
                    ...
                    _instance = new PhimRepository(_default_index);
                }
                return _instance;
            }
        }
        ...
        public List<Phim> GetAll(out long total, int page = 1, int page_size = 99999, string[] view_field = null)
        {
            ...
            {
                new TermQuery{Field="loai", Value=LoaiPhim.PHIM}
            };
        ...
        public long CountTotal()
        {
            ...
            SearchRequest req = new SearchRequest(_default_index)
            {
                Size = 0,
                Query = new QueryContainer(new BoolQuery { Filter = filter }),
                Aggregations = new CardinalityAggregation("CountFilm", "id.keyword")
            };
            var res = client.Search<Phim>(req);
            if (res.IsValid)
            {
                double total_dbl = res.Aggregations.Cardinality("CountFilm").Value ?? 0.0;
