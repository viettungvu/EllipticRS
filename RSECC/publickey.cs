...
namespace RSECC
{
    ...
    public class PublicKey
    {
        ...
        public byte[] toDer()
        {
            ...
            return Utils.Der.encodeSequence(new List<byte[]> {
encodedEcAndOid,
        ...
        public static PublicKey fromDer(byte[] der)
        {
            ...
            {
                throw new ArgumentException("trailing junk after DER public key: " + Utils.BinaryAscii.hexFromBinary(removeSequence1.Item2));
            }
            ...
            {
                throw new ArgumentException("trailing junk after DER public key objects: " + Utils.BinaryAscii.hexFromBinary(removeObject2.Item2));
            }
            ...
            if (!Curves.curvesByOid.ContainsKey(stringOid))
            {
                ...
                CurveType[] supportedCurves = new CurveType[numCurves];
                for (int i = 0; i < numCurves; i++)
                {
                    supportedCurves[i] = Curves.supportedCurves[i].type;
                }
                throw new ArgumentException("Unknown curve with oid [" + string.Join(", ", oidCurve) + "]. Only the following are available: " + string.Join(", ", supportedCurves));
            }
            ...
            return fromString(Utils.Bytes.sliceByteArray(pointString, 2), curve.type);

        }

        public static PublicKey fromString(byte[] str, CurveType curve = CurveType.sec256k1, bool validatePoint = true)
        {
            CurveFp curveObject = Curves.getCurveByType(curve);
